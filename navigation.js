...
(function() {
  ...
  $('nav2-pack-create').onclick=()=>{const title=$('nav2-pack-title').value.trim(),notes=$('nav2-pack-notes').value.trim();if(!title||!notes){$('nav2-pack-status').textContent='Add a title and your journey notes first.';return;}const old=packs();if(old.length>=30){$('nav2-pack-status').textContent='30 packs saved. Export and remove a pack before adding another.';return;}if(savePacks([{id:crypto.randomUUID(),title,notes,created:new Date().toISOString()},...old])){$('nav2-pack-status').textContent='Saved on this device. Open Journeys even without a map connection.';$('nav2-pack-title').value='';$('nav2-pack-notes').value='';renderPacks();}};
